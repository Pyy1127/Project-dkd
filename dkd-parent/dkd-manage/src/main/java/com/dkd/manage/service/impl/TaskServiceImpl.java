...
import com.dkd.common.test.constant.DkdContants;
import com.dkd.common.test.exception.ServiceException;
import com.dkd.common.test.utils.DateUtils;
import com.dkd.common.utils.SerialNumberGenerator;
...
public class TaskServiceImpl implements ITaskService 
{
    ...
    public int insertTask(Task task) {
        if (task.getTaskCode() == null || task.getTaskCode().isEmpty()) {
            task.setTaskCode(SerialNumberGenerator.generateDailySerial(""));
        }
    ...
    public int insertTaskDto(TaskDto taskDto) {
        ...
        //3. 检查设备是否有未完成的同类型工单
        hasTask(vm.getInnerCode(), taskDto.getProductTypeId());
        //4. 查询并校验员工是否存在
        ...
        task.setTaskStatus(Integer.valueOf(DkdContants.TASK_STATUS_CANCEL.toString()));// 创建工单
    ...
    private void hasTask(String innerCode, Long productTypeId) {
        ...
        taskParam.setTaskStatus(Integer.valueOf(DkdContants.TASK_STATUS_PROGRESS.toString()));
    ...
    public int cancelTask(Task task) {
        ...
        //2. 设置更新字段
        task.setTaskStatus(Integer.valueOf(DkdContants.TASK_STATUS_CANCEL.toString()));// 工单状态：取消
